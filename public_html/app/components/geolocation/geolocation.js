var app = angular.module('geoLocation', ['firebase', 'firebase.utils']);
app.factory('geoPosition', ['$firebaseAuth', 'fbutil', '$q', function ($firebaseAuth, fbutil, $q) {
  var utils = {
    map: null,
    init: function () {
      var that = this;
      ...
      if (geoPosition.init()) {
        geoPosition.getCurrentPosition(function (p) {
          that.showPosition(p).then(function (p) {
            console.log(p);
          });
        }, function () {
    ...
    codeLatLng: function (p) {
      var that = this;
      var latitude = parseFloat(p.coords.latitude);
      var longitude = parseFloat(p.coords.longitude);
      var pos = new google.maps.LatLng(latitude, longitude);
      that.geocoder.geocode({'latLng': pos}, function (results, status) {
        if (status == google.maps.GeocoderStatus.OK) {
          if (results[1]) {
            angular.forEach(results[1].address_components, function (value, key) {
              if (value.types[0] === 'locality') {   }
