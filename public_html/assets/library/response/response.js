...
!function (root, name, make) {
  ...
  var Response
    ...
    , defaultPoints = {
      width: [0, 320, 481, 641, 961, 1025, 1281], height: [0, 481], ratio: [1, 1.5, 2] // device-pixel-ratio
    }
    , propTests = {}
    , isCustom = {}
    , sets = { all: [] }
  ...
  // http://github.com/ryanve/verge/issues/13
    ...
    , device = {
      'band': bind(between, deviceW), 'wave': bind(between, deviceH)
    };
  ...
  function merge(r, s) {
    ...
    if (typeof s == 'object' && isNumber(s.length)) push.apply(r, sift(s, 'undefined', true)); else for (var k in s) owns.call(s, k) && void 0 !== s[k] && (r[k] = s[k]);
    return r;
  }
  ...
  function route(item, fn, scope) {
    ...
    if (typeof item == 'object' && !item.nodeType && isNumber(item.length)) each(item, fn, scope); else fn.call(scope || item, item);
    return item;
  }
  ...
  function rectangle(el, verge) {
    ...
    return {
      top: (r.top || 0) - verge, left: (r.left || 0) - verge, bottom: (r.bottom || 0) + verge, right: (r.right || 0) + verge
    };
  }
  ...
  function store($elems, key, source) {
    ...
    route($elems, function (el) {
      if (source) valToStore = el.getAttribute(source); else if (0 < detectMode(el)) valToStore = el.getAttribute('src'); else valToStore = el.innerHTML;
  ...
  Elemset = (function () {
    ...
    return {
      ...
      , newValue: 0, currValue: 1, aka: null, lazy: null, i: 0  // integer, the index of the current highest active breakpoint min
      , uid: null, reset: function () {
