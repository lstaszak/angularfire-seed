...
(function (factory) {
  ...
  $(function () {
    ...
    if ($('#shuffle-grid').length > 0) {
      // Shuffle
      // ================================
      var $grid = $('#shuffle-grid'), $sizer = $grid.find('shuffle-sizer');
      // instatiate shuffle
      $grid.shuffle({
        itemSelector: '.post',
        sizer: $sizer
      ...
      $('html').on('fa.sidebar.minimize', function () {
          $grid.shuffle('update');
    ...
    $('html').on('fa.formajax.done', function (event, data) {
      ...
      data.element.find('.message-container').html(bsalert);
    });
  });
}));
