...
(function (factory) {
  ...
  $(function () {
    // Basic
    // ================================
    var url = 'server/php/', rowId = 0, imgId = 0, uploadButton = $('<button/>').addClass('btn btn-primary').prop('disabled', true).text('Processing...').on('click', function () {
    ...
    $('#basic-uploader input[type="file"]').fileupload({
      ...
      $('#basic-uploader .upload-lists').children('tbody').append('<tr class="upload-list-' + (rowId += 1) + '">' + '<td width="50"></td>' + '<td><strong>' + data.files[0].name + '</strong></td>' + '<td>' + (data.files[0].size * 1e-6) + 'MB</td>' + '<td class="text-right" width="80"></td>' + '</tr>');
      ...
      var index = data.index, file = data.files[index], $canvas = $(file.preview).addClass('img-circle pull-left');
