...
(function (factory) {
  if (typeof define === 'function' && define.amd) {
    define([
      'fastclick', 'responsejs', 'unveil', 'placeholder', 'waypoint', 'waypoint.inview', 'transit', 'slimscroll'
    ], factory);
  } else {
    factory();
  }
  ...
  }, isMinimize = false, isScreenlg = false, isScreenmd = false, isScreensm = false, isScreenxs = false;
  // Core MAIN function
  // ================================
  ...
  MAIN.prototype = {
    ...
    VIEWPORTWATCH: function () {
      ...
      Response.action(function () {
        ...
        if (isScreenlg) {
          $(element).addClass('screen-lg').removeClass('screen-md').removeClass('screen-sm').removeClass('screen-xs');
        }
        if (isScreenmd) {
          $(element).removeClass('screen-lg').addClass('screen-md').removeClass('screen-sm').removeClass('screen-xs');
        }
        if (isScreensm) {
          $(element).removeClass('screen-lg').removeClass('screen-md').addClass('screen-sm').removeClass('screen-xs');
        }
        if (isScreenxs) {
          $(element).removeClass('screen-lg').removeClass('screen-md').removeClass('screen-sm').addClass('screen-xs');
        }
      });
    },
    // Misc
    ...
    PLUGINS: function () {
      ...
      (function () {
        ...
        $(toggler).each(function () {
          var wayShowAnimation = $(this).data('showanim') || 'fadeIn', wayHideAnimation = $(this).data('hideanim') || false, wayOffset = $(this).data('offset') || '80%', wayMarker = $(this).data('marker') || this, triggerOnce = $(this).data('trigger-once') || false;
          // waypoints core
          $(wayMarker).waypoint({
            handler: function (direction) {
              //console.log($(wayMarker));
              if (direction === 'down') {
                $(wayMarker).removeClass(wayHideAnimation + ' animated').addClass(wayShowAnimation + ' animating').on('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function () {
                  $(this).removeClass('animating').addClass('animated').removeClass(wayShowAnimation);
                });
              }
              if ((direction === 'up') && (wayHideAnimation !== false)) {
                $(wayMarker).removeClass(wayShowAnimation + ' animated').addClass(wayHideAnimation + ' animating').on('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function () {
      ...
      (function () {
        ...
        $(toggler).each(function () {
          var inviewEnterClass = $(this).data('enter-class') || 'inview-enter', inviewEnteredClass = $(this).data('entered-class') || 'inview-entered', inviewExitClass = $(this).data('exit-class') || 'inview-exit', inviewExitedClass = $(this).data('exited-class') || 'inview-exited';
          new Waypoint.Inview({
            element: this,
            enter: function (direction) {
              $(this.element).removeClass(inviewEnteredClass).removeClass(inviewExitClass).removeClass(inviewExitedClass).addClass(inviewEnterClass);
            },
            entered: function (direction) {
              $(this.element).removeClass(inviewEnterClass).removeClass(inviewExitClass).removeClass(inviewExitedClass).addClass(inviewEnteredClass);
            },
            exit: function (direction) {
              $(this.element).removeClass(inviewEnterClass).removeClass(inviewEnteredClass).removeClass(inviewExitedClass).addClass(inviewExitClass);
            },
            exited: function (direction) {
              $(this.element).removeClass(inviewEnterClass).removeClass(inviewEnteredClass).removeClass(inviewExitClass).addClass(inviewExitedClass);
            }
          });
        });
      })();
      ...
      // ================================
      (function () {
        var contextual, toggler = '[data-toggle~=selectrow]', target = $(toggler).data('target');
      ...
      // ================================
      (function () {
        var contextual, toggler = '[data-toggle~=checkall]';
        // check on DOM ready
        $(toggler).each(function () {
          if ($(this).is(':checked')) {
            checked();
      ...
      // ================================
      (function () {
        var isDemo = false, indicatorClass = 'indicator', toggler = '[data-toggle~=panelrefresh]';
        // clicker
        $(element).on('click', toggler, function (e) {
          // find panel element
          var panel = $(this).parents('.panel'), indicator = panel.find('.' + indicatorClass);
      ...
      (function () {
        ...
        $(element).on('click', toggler, function (e) {
          // find panel element
          var panel = $(this).parents('.panel'), target = panel.children('.panel-collapse'), height = target.height();
          // error handling
          if (target.length === 0) {
          ...
          var open = function (toggler) {
            ...
            $(target).removeClass('pull').addClass('pulling').css('height', '0px').transition({ height: height }, function () {
          ...
          var close = function (toggler) {
            ...
            $(target).removeClass('pull out').addClass('pulling').css('height', height).transition({ height: '0px' }, function () {
      ...
      // ================================
      (function () {
        var panel, parent, handler = '[data-toggle~=panelremove]';
        // clicker
      ...
      (function () {
        // define variable
        var menuHandler = '[data-toggle~=menu]', submenuHandler = '[data-toggle~=submenu]';
        // core toggle collapse
        var handleClick = function (e) {
          var $this = $(this), parent = $this.data('parent'), target = $this.data('target');
        ...
        var handleHover = function (e) {
          var $this = $(this), parent = $this.children(submenuHandler).data('parent'), target = $this.children(submenuHandler).data('target');
          // run only on tablet view and sidebar-menu collapse
          if ((isScreensm) || (isMinimize)) {
            // if have target
            if ($(target).length > 0) {
        ...
        $(document).on('click', submenuHandler, handleClick).on('mouseenter mouseleave', menuHandler + ' > li', handleHover);
      })();
      // @PLUGIN: SideBar
      // Self invoking
      // ================================
      (function () {
        var direction, sidebar, toggler = '[data-toggle~=sidebar]', openClass = 'sidebar-open';
        var open = function () {
        ...
        $(document).on('click', close).on('click', '.sidebar,' + toggler,function (e) {
          e.stopPropagation();
      ...
      $(function () {
        // define variable
        var container = '[data-toggle~=offcanvas]', pluginErrors = [], direction, optOpenerClass, optCloserClass;
        $(container).each(function (index, value) {
          ...
          if (pluginErrors.length <= 0) {
            $(value).on('click', '.' + optOpenerClass,function (e) {
              // get direction
              ...
              $(value).removeClass('offcanvas-open-ltr offcanvas-open-rtl').addClass(direction);
              // trigger custom event
              $(element).trigger(settings.eventPrefix + '.offcanvas.open', { 'element': $(value) });
              // prevent default
              e.preventDefault();
            }).on('click', '.' + optCloserClass, function (e) {
              $(value).removeClass('offcanvas-open-ltr offcanvas-open-rtl');
              // trigger custom event
